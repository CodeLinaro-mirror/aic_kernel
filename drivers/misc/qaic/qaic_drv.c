...
static long qaic_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case QAIC_IOCTL_WAIT_EXEC_NR:
		...
		break;
	case QAIC_IOCTL_QUERY_NR:
		if (_IOC_DIR(cmd) != (_IOC_READ | _IOC_WRITE) ||
		    _IOC_SIZE(cmd) < sizeof(struct qaic_query)) {
			ret = -EINVAL;
			break;
		}
		ret = qaic_query_ioctl(qdev, usr, arg);
		break;
	default:
		ret = -ENOTTY;
	}
			  ...
			  const struct pci_device_id *id)
{
	...
	for (i = 0; i < QAIC_NUM_DBC; ++i) {
		...
		idr_init(&qdev->dbc[i].dma_handles);
		idr_init(&qdev->dbc[i].alloc_handles);
...
MODULE_VERSION("10.0.1"); /* MAJOR.MINOR.PATCH */
